...
/**
 * 测试用的JVM内部变化消费者，用于北斗广告库
 *
 * @author hexiufeng, zhangxu
 */
public class BeidouAddbConsumer implements Consumer {

    private Logger logger = LoggerFactory.getLogger(BeidouAddbConsumer.class);
    ...
    protected void printTableDef(ChangeDataSet changeDataSet) {
        ...
        logger.info("TableDef: {}", tableDef.keySet());
    }
    ...
    protected void printTableData(ChangeDataSet changeDataSet) {
        ...
        for (String tableName : tableData.keySet()) {
            logger.info("{}->{}", tableName, tableData.get(tableName).size());
        }
    }
