...
public class DecimalColumnDataParser implements ColumnDataParser {
	...
	private static final int powers10[] = { 1, 10, 100, 1000, 10000, 100000,
			1000000, 10000000, 100000000, 1000000000 };

	@Override
	public void parse(ByteBuffer buf, ColumnData columnData, ColumnMeta meta) {
	    int len = getDecimalBinSzie(meta.getMetaValue());
        byte[] decBuf = new byte[len];
        buf.get(decBuf);
		int metaValue = meta.getMetaValue();
		int scale = (metaValue>>>8);
		int precision = metaValue&0xff;
		BigDecimal val = getDecimal(precision,scale,decBuf);
		columnData.setValue(val);
		columnData.setJavaType(Types.DECIMAL);
	}
	 ...
	 private DecimalInfo getDecimalInfo(int precision, int scale) {
		...
		int binSize = intg0 * SIZE_OF_INT32 + dig2bytes[intg0x] + frac0
	...
	private BigDecimal getDecimal(int precision, int scale, byte[] buf) {
		DecimalInfo info = getDecimalInfo(precision, scale);
		BigDecimal decimal = getDecimal0(buf, 0, info.getIntg(),
				info.getFrac(), info.getIntg0(), info.getFrac0(),
				info.getIntg0x(), info.getFrac0x());
		return decimal;
	}

	private BigDecimal getDecimal0(byte[] buffer, final int begin,
			final int intg, final int frac, final int intg0, final int frac0,
			final int intg0x, final int frac0x) {
